...
"""
手动摆牌脚本
玩家a手动摆牌，另一个玩家是AI，游戏记录保存到json文件供AI学习
"""
...
class ManualPlay:
    ...
    def __init__(self, batch_mode=False, batch_steps=None):
        """初始化手动摆牌游戏
        
        Args:
            batch_mode: 是否使用批处理模式
            batch_steps: 批处理步骤，格式为 [(card_idx, area), ...]
        """
        self.game = OFCGame()
    ...
    def setup_game(self):
        """设置游戏"""
        # 创建人类玩家
        self.human_player = Player("PlayerA")
        ...
        print("游戏设置完成，玩家A为手动摆牌，AIPlayer为自动摆牌")
    ...
    def get_user_input(self):
        ...
        if self.batch_mode:
            # 批处理模式下，使用预设的步骤
            if self.current_step < len(self.batch_steps):
                card_idx, area = self.batch_steps[self.current_step]
                self.current_step += 1
    ...
    def play_game(self):
        ...
        # 计算得分
        for player in self.game.players:
            score = self.game.calculate_total_score(player)
            is_busted = self.game.check_busted(player)
            print(f"{player.name} 的得分: {score} (爆牌: {is_busted})")
...
if __name__ == "__main__":
    # 默认使用批处理模式，预设一些摆牌步骤
    # 示例：将前3张牌放到top区域，中间5张放到middle区域，后5张放到bottom区域
    batch_steps = [
        (0, 'top'),    # 移动第1张牌到top区域
        ...
        (0, 'middle'), # 移动第3张剩余牌到middle区域
        (0, 'middle'), # 移动第4张剩余牌到middle区域
        (0, 'middle'), # 移动第5张剩余牌到middle区域
        (0, 'bottom'), # 移动第1张剩余牌到bottom区域
        (0, 'bottom'), # 移动第2张剩余牌到bottom区域
        (0, 'bottom'), # 移动第3张剩余牌到bottom区域
        (0, 'bottom'), # 移动第4张剩余牌到bottom区域
        (0, 'bottom'), # 移动第5张剩余牌到bottom区域
    ]
    
    game = ManualPlay(batch_mode=True, batch_steps=batch_steps)
    game.play_game()
